...
import { JUEGO3_ROUNDS } from '../../data/juego3Config' // 📦 Importación de la nueva configuración
...
export default function Juego1View({ player, timeLeft, onPointsUpdate, onGameFinished }: Juego1ViewProps) {
  ...
  const [currentRound, setCurrentRound] = useState<number>(() => {
    ...
    return parsed >= JUEGO3_ROUNDS.length ? 0 : parsed
  })
  ...
  const activeRoundData = JUEGO3_ROUNDS[currentRound] || JUEGO3_ROUNDS[0]
  ...
  const handleSelectOption = (idxSelected: number) => {
    ...
    if (nextRound < JUEGO3_ROUNDS.length) {
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        <span>Pregunta {currentRound + 1} de {JUEGO3_ROUNDS.length}</span>
        {timeLeft !== null && (
